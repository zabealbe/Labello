...
class Gui {
    constructor() {
        ...
        this.onResize = call(this.onResize)
        ...
        // on canvas parent update
        canvas.value.parentElement.addEventListener('resize', this.onResize)
    }
    ...
    async add(d, visible = true) {
        ...
        switch (d.constructor) {
            ...
            case LabelRectangle:
                shape = GuiRectangle.fromLabel(d, options)
                break;
            case LabelRotatedRectangle:
                shape = GuiRotatedRectangle.fromLabel(d, options)
                break;
            case LabelPolygon:
                shape = GuiPolygon.fromLabel(d, options)
                break;
            default:
...
class GuiPolygon extends paper.Path {
    ...
    set points(points) {
        for (let i = 0; i < this.segments.length; i++) {
            ...
            this.__children[i].position = points[i]
        }
    }
    ...
    scale(scale, origin) {
        ...
        this.__children.forEach((corner) => {
    ...
    static fromLabel(label) {
        ...
        return new this.constructor(label.points.map((p) => p.multiply(gui.value.sceneImage.bounds.size).add(gui.value.sceneImage.topLeft)))
    }
    toLabel() {
        return new LabelPolygon(this.points.map((p) => p.subtract(gui.value.sceneImage.topLeft).divide(gui.value.sceneImage.bounds.size)))
    }

    remove() {
        this.__children.forEach((corner) => corner.remove())
        super.remove()
    }
}
...
function loadSample(id) {
    ...
    return gui.value.addBatch([new LabelImage(files[id]), ...(labels[files[id].name] || [])]).then(() => {
